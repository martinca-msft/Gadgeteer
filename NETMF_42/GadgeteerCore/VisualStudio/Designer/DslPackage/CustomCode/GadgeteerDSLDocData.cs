...
namespace Microsoft.Gadgeteer.Designer
{
	...
	partial class GadgeteerDSLDocData
	{
		...
		{			
            ...
            if (diagram != null)
                diagram.AddToolboxFilter(new ToolboxItemFilterAttribute(GadgeteerDSLToolboxHelperBase.ToolboxFilterString + ProjectMFVersion, ToolboxItemFilterType.Require));
		...
		private void SetupAssemblyReferencing()
		{
			...
			if (project != null)
			{
                ...
                vsproject.Events.ReferencesEvents.ReferenceAdded += ReferencesEvents_ReferenceChangeHandler;
                vsproject.Events.ReferencesEvents.ReferenceRemoved += ReferencesEvents_ReferenceChangeHandler;
                vsproject.Events.ReferencesEvents.ReferenceChanged += ReferencesEvents_ReferenceChangeHandler;
        ...
        private void StoreGadgeteerCoreVersionNumber()
        {
            ...
            if (gadgeteerRef != null && !string.IsNullOrWhiteSpace(gadgeteerRef.Path))
            {
                ...
                this.Store.PropertyBag[GadgeteerModel.GadgeteerCoreVersionKey] = new Version(ver.FileVersion);
            }
            else
            {

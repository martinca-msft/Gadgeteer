...
namespace Gadgeteer.Networking
{
    ...
    public class Responder
    {
        ...
        /// </summary>
        public WebServer.HttpMethod HttpMethod { get; internal set; }
        ...
        public Body Body
        {
            get
            {
                if (HttpMethod == WebServer.HttpMethod.POST || HttpMethod == WebServer.HttpMethod.PUT)
                {
                    if (_Body == null)
                    {
        ...
        {
            // Process Header 
            string request = new string(Encoding.UTF8.GetChars(HeaderData));
            ...
            for (int i = 0; i < splittedRequest.Length; i++)
            {
                ...
                if (line.Length > 0 && !line.Equals(" "))
                {
                    ...
                    if (i == 0)
                    {
                        ...
                        {
                            contentLength = 0;
                            HttpMethod = WebServer.HttpMethod.GET;
                        }
                        else if (MethodAsString.Equals("POST"))
                        {
                            HttpMethod = WebServer.HttpMethod.POST;
                        }
                        else if (MethodAsString.Equals("PUT"))
                        {
                            HttpMethod = WebServer.HttpMethod.PUT;
                        }
                        else if (MethodAsString.Equals("DELETE"))
                        {
                            contentLength = 0;
                            HttpMethod = WebServer.HttpMethod.DELETE;
                        }
                        ...
                        else
                        {
                            ...
                            if (HttpMethod == WebServer.HttpMethod.DELETE || HttpMethod == WebServer.HttpMethod.GET)
                            {
                                ...
                                if (array.Length == 2)
                                {
                                    ...
                                    foreach (string tuple in parameterTuples)
                                    {
                                        string[] parameterData = tuple.Split('=');
                        ...
                        HTTPVersion = httpFirstLineParameters[2];
                    }
                    else
                    {

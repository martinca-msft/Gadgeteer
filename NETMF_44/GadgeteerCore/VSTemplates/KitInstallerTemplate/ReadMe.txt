...
(normally 5 projects, one for NETMF 4.1, one for NETMF 4.2, one for NETMF 4.3, one for NETMF 4.4 and one for the installer merge module).  
  ...
  named NETMF41, NETMF42, NETMF43, NETMF44, MergeModules. Then, for each of your modules/mainboards, put its three projects into the relevant folders.
...
its NETMF41, NETMF42, NETMF43 and NETMF44 projects, and ensure that the Kit WiX project relies on all the module/mainboard merge module installer projects.
...
If you rebuild in Release configuration, then every module/mainboard will be rebuilt and then the kit installer will be rebuilt, which can take a long time.
...
SEPARATE, which is used by default, means the kit and every module and mainboard to have separately managed version numbers.
	...
	- One for each version of NETMF supported (e.g. one for NETMF 4.1, one for NETMF 4.2, one for NETMF 4.3 and one for NETMF 4.4)
...
SYNCHRONIZED involves one-off manual edits to each module/mainboard to cause them to always synchronize with a Kit's version number.
	...
	- For the NETMF projects (three if the module/mainboard supports all NETMF 4.1, NETMF 4.2, NETMF 4.3 and NETMF 4.4),
